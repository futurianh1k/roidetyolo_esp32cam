...
void reconnectMQTT() {
  ...
  // Last Will Testament (LWT) 설정 - 장비가 예기치 않게 연결 끊길 때 자동으로
  // 오프라인 메시지 전송
  String lwt_topic = String(TOPIC_STATUS);
  String lwt_message =
      "{\"device_id\":\"" + String(DEVICE_ID) +
      "\",\"online\":false,\"timestamp\":" + String(millis() / 1000) + "}";

  if (mqttClient.connect(clientId.c_str(), MQTT_USERNAME, MQTT_PASSWORD,
                         lwt_topic.c_str(), MQTT_QOS, true,
                         lwt_message.c_str())) {
    ...
    mqttClient.subscribe(TOPIC_CONTROL_SYSTEM, MQTT_QOS);
