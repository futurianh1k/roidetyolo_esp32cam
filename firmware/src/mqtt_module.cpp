...
/**
 * 마이크 제어 처리 (ASR 모드 및 일반 오디오 WebSocket 전송 포함)
 *
 * @param action 액션 (start, pause, stop, start_asr, stop_asr)
 * @param requestId 요청 ID
 * @param sessionId ASR 세션 ID (start_asr일 때만 사용)
 * @param wsUrl WebSocket URL (start_asr일 때 또는 일반 마이크 start에서 선택사항)
 */
                             ...
                             const char *sessionId, const char *wsUrl)
{
  ...
  if (strcmp(action, "start") == 0)
  {
    ...
    if (audioStartMicrophone())
    {
      ...
      // WebSocket URL이 있으면 오디오를 WebSocket으로 전송하도록 설정
      if (wsUrl)
      {
        // 간단한 session ID 생성 (일반 마이크용)
        String sessionId = "mic_" + String(millis());

        if (websocketConnect(sessionId.c_str(), wsUrl))
        {
          DEBUG_PRINTF("✅ 오디오 WebSocket 연결 성공: %s\n", wsUrl);
        }
        else
        {
          DEBUG_PRINTF("❌ 오디오 WebSocket 연결 실패: %s\n", wsUrl);
        }
      }

      displayShowStatus("Mic ON", TFT_GREEN);
      DEBUG_PRINTLN("✅ 일반 마이크 시작 완료");
    }
    else
    {
      message = "Microphone start failed";
      DEBUG_PRINTLN("❌ 일반 마이크 시작 실패");
    }
  }
