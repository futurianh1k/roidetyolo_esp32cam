...
## Positive/Negative 테스트 케이스

테스트 스크립트는 두 가지 타입의 테스트를 실행합니다:
...
예외 상황 및 에러 핸들링을 검증하는 테스트:

- 잘못된 자격증명으로 로그인 시도 → 401 예상
- 유효하지 않은 토큰으로 인증 시도 → 401 예상
- 약한 비밀번호로 사용자 등록 시도 → 400 예상
- 존재하지 않는 장비 조회 → 404 예상
- 중복 장비 ID로 등록 시도 → 400 예상
- 잘못된 IP 형식으로 장비 등록 시도 → 400/422 예상
- 오프라인 장비 제어 시도 → 400 예상
- 잘못된 액션으로 제어 시도 → 400/422 예상
- 필수 파라미터 누락 → 400 예상
- 잘못된 파일 형식 업로드 → 400 예상

**Negative 테스트의 목적**: 백엔드 API가 예외 상황을 올바르게 처리하고 적절한 HTTP 상태 코드를 반환하는지 검증합니다.
