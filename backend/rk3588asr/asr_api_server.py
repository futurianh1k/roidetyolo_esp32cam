...
class SessionStartRequest(BaseModel):
    ...
    language: str = Field(default="auto", description="언어 코드 (auto, ko, en, zh, ja, yue)")
...
class ASRSession:
    """음성인식 세션"""
    def __init__(self, session_id: str, device_id: str, language: str = "auto", 
        ...
        if recognizer is None:
            raise RuntimeError("❌ Recognizer가 초기화되지 않았습니다. load_model()을 먼저 실행하세요.")
    ...
    async def process_audio_chunk(self, audio_data: np.ndarray) -> Optional[Dict]:
        ...
        if result:
            ...
            if text:
                ...
                result['emergency_keywords'] = match_result.get('emergency_keywords', [])
...
class SessionManager:
    ...
    def create_session(self, device_id: str, language: str = "auto", 
                      sample_rate: int = 16000, vad_enabled: bool = True) -> ASRSession:
        """새 세션 생성"""
    ...
    def remove_session(self, session_id: str):
        ...
        if session_id in self.sessions:
            ...
            logger.info(f"🗑️ 세션 제거: {session_id} (남은 세션: {len(self.sessions)}개)")
...
async def websocket_asr_endpoint(websocket: WebSocket, session_id: str):
    ...
    if not session:
        await websocket.close(code=4004, reason=f"세션을 찾을 수 없습니다: {session_id}")
        return
