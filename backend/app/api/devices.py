...
async def list_devices(
    ...
    # 페이지네이션
    devices = query.order_by(desc(Device.registered_at)).offset((page - 1) * page_size).limit(page_size).all()
...
async def update_device(
    ...
    if device_data.ip_address is not None:
        ...
        # RTSP URL 업데이트
        device.rtsp_url = f"rtsp://{device_data.ip_address}:554/stream"
    ...
    logger.info(f"사용자 {current_user.username}가 장비 {device.device_name} 정보 수정")
...
@router.post("/{device_id}/status", response_model=DeviceStatusResponse, status_code=status.HTTP_201_CREATED)
...
async def get_device_latest_status(
    ...
    # 최신 상태 조회
    latest_status = db.query(DeviceStatus).filter(
        DeviceStatus.device_id == device_id
