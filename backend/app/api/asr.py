...
@router.post("/devices/{device_id}/session/start", response_model=ASRSessionStartResponse)
async def start_device_asr_session(
    ...
    """
    logger.info(f"음성인식 세션 시작 요청: device_id={device_id}, language={request.language}")
    ...
    if device_id in active_sessions:
        ...
        logger.warning(f"⚠️ 이미 활성 세션 존재: device_id={device_id}, session_id={existing_session_id}")
        raise HTTPException(
    ...
    try:
        ...
        logger.info(f"✅ 음성인식 세션 시작 완료: device={device.device_name}, session={session_id}")
...
async def stop_device_asr_session(
    ...
    """
    logger.info(f"음성인식 세션 종료 요청: device_id={device_id}, session_id={request.session_id}")
    ...
    if stored_session_id != request.session_id:
        logger.warning(f"⚠️ 세션 ID 불일치: stored={stored_session_id}, requested={request.session_id}")
    ...
    try:
        ...
        logger.info(f"✅ 음성인식 세션 종료 완료: device={device.device_name}, session={request.session_id}")
...
@router.get("/devices/{device_id}/session/status", response_model=ASRSessionStatusResponse)
