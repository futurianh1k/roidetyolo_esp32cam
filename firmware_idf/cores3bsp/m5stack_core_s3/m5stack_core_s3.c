 ...
 *
 * You can call i2c_master_get_bus_handle(BSP_I2C_NUM, i2c_master_bus_handle_t
 * *ret_handle) from #include "esp_private/i2c_platform.h"
 */
...
esp_err_t bsp_i2c_init(void) {
  ...
  // Prefer reusing an already-created bus handle to avoid noisy error logs
  // and prevent I2C bus acquisition failures when another module owns the port.
  if (i2c_master_get_bus_handle(BSP_I2C_NUM, &i2c_handle) == ESP_OK) {
    ESP_LOGW(TAG, "I2C bus already acquired, reusing existing bus handle");
  } else {
  ...
  BSP_ERROR_CHECK_RETURN_ERR(
      i2c_master_bus_add_device(i2c_handle, &axp2101_config, &axp2101_h));
  ...
  BSP_ERROR_CHECK_RETURN_ERR(
      i2c_master_bus_add_device(i2c_handle, &aw9523_config, &aw9523_h));
...
sdmmc_card_t *bsp_sdcard_get_handle(void) { return bsp_sdcard; }
...
esp_err_t bsp_sdcard_sdspi_mount(bsp_sdcard_cfg_t *cfg) {
  ...
  ESP_RETURN_ON_ERROR(
      bsp_spi_init((BSP_LCD_H_RES * BSP_LCD_V_RES) * sizeof(uint16_t)), TAG,
      "");
  ...
  ESP_LOGW(TAG,
           "Warning: Long filenames on SD card are disabled in menuconfig!");
#endif
...
esp_err_t bsp_display_brightness_init(void) {
  ...
  ESP_RETURN_ON_ERROR(
      i2c_master_transmit(axp2101_h, lcd_bl_en, sizeof(lcd_bl_en), 1000), TAG,
      "I2C write failed");
  const uint8_t lcd_bl_val[] = {0x99, 0b00011000}; // AXP DLDO1 voltage
  ESP_RETURN_ON_ERROR(
      i2c_master_transmit(axp2101_h, lcd_bl_val, sizeof(lcd_bl_val), 1000), TAG,
      "I2C write failed");
...
esp_err_t bsp_display_brightness_set(int brightness_percent) {
  ...
  const uint8_t reg_val =
      20 + ((8 * brightness_percent) /
            100); // 0b00000 ~ 0b11100; under 20, it is too dark
...
esp_err_t bsp_touch_new(const bsp_touch_config_t *config,
  ...
  tp_io_config.scl_speed_hz =
      CONFIG_BSP_I2C_CLK_SPEED_HZ; // This parameter was introduced together
                                   // with I2C Driver-NG in IDF v5.2
  ESP_RETURN_ON_ERROR(
