...
bool MQTTClient::Connect(const std::string& client_id) {
    ...
    esp_mqtt_client_config_t* cfg = (esp_mqtt_client_config_t*)mqtt_client_;
    // Note: 실제로는 esp_mqtt_client_set_config를 사용해야 하지만,
    // 여기서는 간단히 esp_mqtt_client_start 사용
...
void MQTTClient::MQTTEventHandler(void* handler_args, esp_event_base_t base,
    ...
    switch (event->event_id) {
        ...
        case MQTT_EVENT_ERROR:
            ESP_LOGE(TAG, "MQTT error: %s", event->error_handle->error_type == MQTT_ERROR_TYPE_ESP_TLS ?
                     event->error_handle->esp_tls_last_esp_err : "Unknown");
            break;
