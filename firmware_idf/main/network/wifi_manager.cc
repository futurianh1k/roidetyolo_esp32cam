...
bool WiFiManager::Initialize(const std::string &ssid,
...
void WiFiManager::OnWiFiEvent(esp_event_base_t event_base, int32_t event_id,
                              void *event_data) {
  if (event_base == WIFI_EVENT) {
    ...
    case WIFI_EVENT_STA_DISCONNECTED: {
      ...
      ESP_LOGW(TAG, "Disconnected from AP: %s, reason: %d", event->ssid,
               event->reason);
      connected_ = false;
